...
export default function Sidebar({ newChat, conversations, loadConversation, currentConversationId, isMobileSidebarOpen, setIsMobileSidebarOpen }) {
  const { user } = useUser();
  ...
  return (
    <SignedIn>
      ...
      {/* Mobile Sidebar */}
      {isMobileSidebarOpen && (
        <aside className="md:hidden fixed left-0 top-0 h-full w-[260px] z-50 flex flex-col border-r border-black/[.08] dark:border-white/[.06] bg-[#f7f7f7] dark:bg-[#0f0f0f]">
          ...
          <div className="flex items-center justify-between p-4 border-b border-black/[.08] dark:border-white/[.06]">
            ...
            <button 
              onClick={() => setIsMobileSidebarOpen(false)}
              className="p-1 hover:bg-black/[.05] dark:hover:bg-white/[.1] rounded"
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" className="text-black/60 dark:text-white/60">
